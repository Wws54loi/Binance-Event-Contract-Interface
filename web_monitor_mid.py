    ...
    def check_price(self, price):
        with self.lock:
            ...
            if levels["s_res"] > 0 and prev < levels["s_res"] and price >= levels["s_res"]:
                ...
                allow_trade = True
                if self.recent_klines:
                    if self.recent_klines[-1]['c'] >= levels["s_res"]:
                        allow_trade = False
                        session.log(f"⚠️ 忽略强压力位: 前K线收盘价 {self.recent_klines[-1]['c']} >= {levels['s_res']}")
            ...
            if levels["s_sup"] > 0 and prev > levels["s_sup"] and price <= levels["s_sup"]:
                ...
                allow_trade = True
                if self.recent_klines:
                    if self.recent_klines[-1]['c'] <= levels["s_sup"]:
                        allow_trade = False
                        session.log(f"⚠️ 忽略强支撑位: 前K线收盘价 {self.recent_klines[-1]['c']} <= {levels['s_sup']}")
    ...
    def settle_trade(self, session, trade, current_price):
        ...
        if not is_win and session.is_active:
            ...
            if trade["level_key"] == "s_res":
                ...
                session.stop("强支撑位跌破")
            # 中介线失败是否停止？用户未指定，暂时不停止，或者可以视为弱支撑/压力
...
with st.sidebar:
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("📥 服务器保存", use_container_width=True):
            if bot.save_to_disk():
                st.success("已保存")
            else:
                st.error("保存失败")
    with c2:
        if st.button("📤 服务器加载", use_container_width=True):
